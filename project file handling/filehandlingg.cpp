...
void mainHeader();
int mainmenu();
bool adminlogin(int attempts);
bool stafflogin(int totalstaff);
void adminmenu();
void staffmenu();
void manageStaff();
void addPet();
void viewAllPets(int totalPets);
void searchPet(string name, int totalPets);
void updatePetRecord(string name, int totalPets);
void deletePetRecord(string name, int totalPets);
void bookAppointment(string name, int totalPets);
void viewAppointments(int totalPets);
void addTreatmentRecord(string name, int totalPets);
void viewTreatmentRecords(int totalPets);
float calculatebill(float CFee, float MFee, float TFee);
void generateBill(int totalPets);
void printbill(int i, float CFee, float MFee, float TFee);
void viewBillingRecords(int totalPets);
void assignVet(string name, int totalPets);
void viewAssignedVets(int totalPets);
void viewHistoryLogs(int totalPets);
void viewPetsByOwner(string Ownname, int totalPets);
void sortPetsByAge(int totalPets);
void addStaff();
void viewAllStaff(int totalstaff);
void deleteStaff();
// prototypes END
...
int main() // main function START
{
    ...
    while (true)
    {
        ...
        {
            if (adminlogin(3)) // ask admin login
            {
        ...
        {
            if (stafflogin(staffindex)) // ask staff login
            {
...
void adminmenu()
{
    while (true)
    {
        ...
        else if (opt == 4)
        {
            ...
            getline(cin, name);
            deletePetRecord(name, petindex);
        }
        else if (opt == 5)
            viewAppointments(petindex);
        else if (opt == 6)
            viewTreatmentRecords(petindex);
        else if (opt == 7)
            viewBillingRecords(petindex);
        else if (opt == 8)
            viewAssignedVets(petindex);
        else if (opt == 9)
            viewHistoryLogs(petindex);
        else if (opt == 10)
            manageStaff();
        else if (opt == 11)
            break;
...
void staffmenu()
{
    while (true)
    {
        ...
        else if (opt == 6)
        {
            ...
            cout << "enter name: ";
            getline(cin, name);
            addTreatmentRecord(name, petindex);
        }
        else if (opt == 7)
            generateBill(petindex);
        else if (opt == 8)
        {
            string name;
            cin.ignore();
        ...
        else if (opt == 9)
        {
            ...
            getline(cin, Ownname);
            viewPetsByOwner(Ownname, petindex);
        }
        else if (opt == 10)
            sortPetsByAge(petindex);
        else if (opt == 11)
            break;
        else
        {
...
{
    system("cls");
    cout << "Pet's Name\tAge(in months)\tBreed\tOwner's Name\tDisease" << endl;
    cout << "---------------------------------------------------------------" << endl;
    if (totalPets == 0)
    {
...
void generateBill(int totalPets)
{
    ...
    for (int i = 0; i < totalPets; i++)
    {
        if (petnameA[i] == name)
        {
            ...
            cin >> TFee;
            totalbillA[i] = calculatebill(CFee, MFee, TFee);
            visitA[i]++;
            savepet();
            printbill(i, CFee, MFee, TFee);
            break;
        }
    }
...
void sortPetsByAge(int totalPets)
{
    ...
    for (int i = 0; i < totalPets - 1; i++)
    {
        for (int j = i + 1; j < totalPets; j++)
        {
